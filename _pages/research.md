...
2. [Feedback control of Cortical Dynamics](#feedback-control-of-cortical-dynamics)
3. [Data-driven models for control of biomechanical bodies](#data-driven-models-for-control-of-biomechanical-bodies)
4. [Population dynamics in the cerebellar cortex](#population-dynamics-in-the-cerebellar-cortex)
    - [Dynamics of electrically-coupled cerebellar inhibitory networks](#dynamics-of-electrically-coupled-cerebellar-inhibitory-networks)
    - [Sensorimotor transformation across the cerebellar circuit](#sensorimotor-transformation-across-the-cerebellar-circuit)
5. [Role of perceptual uncertainty in reward-driven learning](#role-of-perceptual-uncertainty-in-reward-driven-learning)
...
<img align="left" src="../../images/dynest.png" alt="Probabilistic model to estimate latent neural trajectories and structure of latent dynamics from neural recordins across days. Dynamics are assumed to be shared across days but mapping from latents to single neural spiking is flexible across days." title="Data-driven estimation of shared latent dynamics." hspace=20px vspace=5px width="30%" />
Identifying low-dimensional latent dynamics describing the lawful evolution of neural population activity over time can powerfully link neural circuit organization to the behavioral computations they support. However, reliably estimating and interpreting these dynamics in complex tasks faces two crucial challenges: (1) balancing model flexibility with interpretability and reducing solution degeneracy, and (2) overcoming the limitations of short recording sessions and/ non-stationarities/drift in chronic recordings in tasks with a lot of heterogeneous conditions and multi-dimensional behavioral variability, even without neuron matching or behavioral supervision. We developed a method that learns a shared nonlinear, low-dimensional dynamical system across sessions that not only captures population co-variability but learns correct dynamical solutions that enable forecasting. 

In collaboration with Christine Constantinople, we applied this approach to electrophysiological recordings from orbitofrontal cortex (OFC) in rats performing a value-guided decision-making task, where rats perform contextual inference over trials to adapt behavior on slow timescales, as well as show within-trial deliberations over tens of seconds, leading to a highly heterogeneous set of trials. Joint optimization with a shared dynamical prior outperformed standard alignment methods such as canonical correlation analysis (CCA), uncovering modulation of latent dynamics on single trials as well as across diverse conditions — time, belief states, stimuli, and decisions. Combining data across sessions allowed us to study rare events like block transitions (context switches), revealing that OFC activity reflects slow updating of “belief about the current block” across multiple trials, and how this timescale of belief updating relates to behavioral adaptation. Furthermore, large variability in an internally-generated continuous decision variable—the time to “opt out” (wait time)—was explained by variability in the speed of OFC neural dynamics. Our approach to dynamics-based alignment across sessions using a well-conditioned yet flexible model offers the opportunity to study richer behavioral variability and multiplexed computations in complex, or even more naturalistic, tasks, as well as track neural representations across days.
